...
WHERE tb_mengajar.id_mengajar='$_GET[pelajaran]' AND tb_mengajar.id_mkelas='$_GET[kelas]'  AND tb_thajaran.status=1 AND tb_semester.status=1
	...
	$walikelas = mysqli_query($con, "SELECT * FROM tb_walikelas INNER JOIN tb_asisten ON tb_walikelas.id_asisten=tb_asisten.id_asisten WHERE tb_walikelas.id_mkelas='$_GET[kelas]' ");
